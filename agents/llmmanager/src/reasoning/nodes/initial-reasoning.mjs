...
import {
  buildContext,
  formatContextPrompt,
} from "../../utils/build-context.mjs";

/**
 * LangGraph-compatible reasoning node. Builds context and prompts an LLM to reason through a decision request.
 * This function does NOT make a final decision.
 *
 * @param {object} state - Should include `query`
 * @param {object} config - LangGraphRunnableConfig with access to `store` and `assistantId`
 * @returns {Promise<object>} - Object with `generatedReasoning` and `promptContext`
 */
export async function initialReasoning(state, config) {
  ...
  return {
    generatedReasoning: response.content,
